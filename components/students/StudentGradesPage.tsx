...
const StudentGradesPage: React.FC = () => {
    ...
    const { grades: allGrades } = useData();
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold mb-4 text-text-primary dark:text-white">Mis Calificaciones</h1>
            {Object.keys(gradesBySubject).length > 0 ? (
                ...
                <Card className="text-center py-10">
                    <p className="text-sm text-text-secondary">Aún no tienes calificaciones registradas.</p>
                </Card>
            )}
        </div>
    );
